...
echo 'source ~/.login-ubuntu' >> ~/.shortcuts/wine-desktop &&
echo 'source /usr/local/bin/vnc-start' >> ~/.shortcuts/wine-desktop &&
echo 'source /usr/local/bin/wine-desktop' >> ~/.shortcuts/wine-desktop &&
source ~/.login-ubuntu
...
echo "DONE"
echo "Restart Termux, and then run ubuntu to login to the proot. In the proot, run vnc-start to open the vnc server, and vnc-stop to close it. Once logged into vnc, run wine-desktop to start it up. If it fails, delete ~/.wine and try running gst wine-desktop instead."
