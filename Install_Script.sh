...
echo "DONE"
echo "Restart Termux, and then run debian to login to the proot. In the proot, run vnc-start to open the vnc server, and vnc-stop to close it. Once logged into vnc, run wine-desktop to start it up. If it fails with a kernel32.dll error, delete ~/.wine and try running gst before wine-desktop."
